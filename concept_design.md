...
## Requirements
REQ-1 - Sensor nodes - the system will support multiple sensor nodes. There will be at least one for temperature and humidity and pressure, one for precipitation, and one for wind. More nodes will be added as desired.

REQ-2 - Sensor data acquisition - Sensor values will be read every 10 s for temperature and humidity and pressure, or as appropriate for the particular variable.
...
REQ-4 - Sensor Node Communications protocol - Sensor nodes will transmit readings to the web server via REST API (HTTP).
...
REQ-6 - Observations and Daily weather records - The system will record daily minima and maxima (temperatures) and precipitation. These values will be stored as daily records in the database.
...
REQ-13 - Data validation and integrity - the web application will validate the received data and discard or flag values that fall outside realistic physical limits.

REQ-14 - Node heartbeat - each node will send a periodic heartbeat message every 60 s to confirm connectivity even when measurements are stable.